...
# Input position repeatibility setpoint, check to ensure it is within sensible limits
...
try:
    plc.write_by_name("MAIN.fbPLOOP.fSetpointValue", sp, pyads.PLCTYPE_REAL)
except:
...
startTime = dt.now()
fileNameTS = HPT_NAME + "-PosRep-" + startDate.strftime("%Y%m%d") + "_" + startTime.strftime("%Hh%Mm")
...
with open(fullFileName, 'w', newline='') as PLC_file:
    ...
    print("Setpoint: " + str(sp))
    plc.write_by_name('MAIN.fbPLOOP.fSetpointValue', sp, pyads.PLCTYPE_REAL)
    while ((monotonic() - t0) >= 30) and ((monotonic() - t0) < 60):
    ...
    print("Setpoint: " + str(sp))
    plc.write_by_name('MAIN.fbPLOOP.fSetpointValue', sp, pyads.PLCTYPE_REAL)
    while ((monotonic() - t0) >= 60) and ((monotonic() - t0) < 90):
    ...
    print("Setpoint: " + str(sp))
    plc.write_by_name('MAIN.fbPLOOP.fSetpointValue', sp, pyads.PLCTYPE_REAL)
    while ((monotonic() - t0) >= 90) and ((monotonic() - t0) < 120):
    ...
    print("Setpoint: " + str(sp))
    plc.write_by_name('MAIN.fbPLOOP.fSetpointValue', sp, pyads.PLCTYPE_REAL)
    while ((monotonic() - t0) >= 120) and ((monotonic() - t0) < 150):
...
# Disable the test variable on the TwinCAT side to disable the HMI button
plc.write_by_name('MAIN.tglPLoop', False, pyads.PLCTYPE_BOOL)
